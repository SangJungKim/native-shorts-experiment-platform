-- Preliminary participant export support: explicit researcher-readable video
-- titles and a lightweight activation event. Exact export schemas remain V0.

alter type public.behavior_event_type add value if not exists 'post_activated' before 'swipe_up';

alter table public.posts add column video_title text not null default '';
update public.posts set video_title = youtube_video_id where btrim(video_title) = '';
alter table public.posts add constraint posts_video_title_not_blank check (length(btrim(video_title)) > 0);

create function public.populate_post_video_title()
returns trigger language plpgsql set search_path = '' as $$
begin
  if btrim(new.video_title) = '' then
    select p.video_title into new.video_title
    from public.posts p
    where p.experiment_id = new.experiment_id and p.youtube_video_id = new.youtube_video_id
    order by p.updated_at desc limit 1;
    new.video_title := coalesce(nullif(btrim(new.video_title), ''), new.youtube_video_id);
  end if;
  return new;
end;
$$;

create trigger populate_post_video_title
before insert on public.posts
for each row execute function public.populate_post_video_title();

alter table public.publication_posts add column video_title text not null default '';

create function public.snapshot_publication_post_title()
returns trigger language plpgsql security definer set search_path = '' as $$
begin
  if btrim(new.video_title) = '' and new.source_post_id is not null then
    select p.video_title into new.video_title from public.posts p where p.id = new.source_post_id;
  end if;
  return new;
end;
$$;

create trigger snapshot_publication_post_title
before insert on public.publication_posts
for each row execute function public.snapshot_publication_post_title();
...
begin
  ...
  if target_event_type not in ('post_activated', 'like', 'unlike', 'comments_opened', 'comments_closed', 'share_tapped', 'share_untapped', 'swipe_up', 'swipe_down') then
  ...
  if target_event_type = 'like' then
    ...
    update public.participant_post_state set share_tapped = true, reposted = true
    where session_id = target_session.id and publication_post_id = target_publication_post_id;
  elsif target_event_type = 'share_untapped' then
    update public.participant_post_state set reposted = false
    where session_id = target_session.id and publication_post_id = target_publication_post_id;
  elsif target_event_type = 'comments_opened' then
    update public.participant_post_state set comments_opened_count = comments_opened_count + 1
    where session_id = target_session.id and publication_post_id = target_publication_post_id;
  end if;
