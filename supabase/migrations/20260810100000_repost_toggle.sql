-- Keep current repost state reversible while retaining whether sharing was ever
-- tapped for analysis. Researcher-configured display counts remain immutable.

alter type public.behavior_event_type add value if not exists 'share_untapped' after 'share_tapped';

alter table public.participant_post_state
...
begin
  ...
  if target_event_type not in ('like', 'unlike', 'comments_opened', 'comments_closed', 'share_tapped', 'share_untapped', 'swipe_up', 'swipe_down') then
