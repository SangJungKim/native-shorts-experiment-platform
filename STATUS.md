...
- Refined the shared participant feed affordances with one original, consistent SVG icon family across iOS, Android, and web: a rounded outlined heart that fills when liked, a circular speech bubble with a tail, and paired repost arrows with a selected-state check badge. The controls retain accessible labels, transparent hit areas, configured-count-plus-one feedback, undo behavior, and the existing research event semantics. Because the native and web experiences share this screen, the change applies to both. Added Expo-compatible `react-native-svg` 15.15.4 to render the controls consistently.
