...
- Refined the shared participant feed affordances for a familiar short-video social interface: an outlined heart that fills when liked, a tailed speech bubble for comments, and a two-arrow repost control that changes color when selected. The controls retain accessible labels, transparent hit areas, configured-count-plus-one feedback, undo behavior, and the existing research event semantics. Because the native and web experiences share this screen, the change applies to both.
