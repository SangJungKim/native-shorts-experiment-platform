...
Milestone 5D external researcher-demo deployment is in progress before returning to Milestone 6. Hosted Supabase staging project `emffazwlvifexhklhdll` is linked; all 22 repository migrations were applied, local and remote migration histories match, and remote schema lint reports no errors. Hosted anonymous authentication is enabled, and researcher UID `a9dd4f3f-d6ca-45bd-8f79-e6f37b4b2c9a` is provisioned in `public.researcher_profiles`. Repository-side EAS profiles and the staging deployment guide are prepared. The private GitHub repository `SangJungKim/native-shorts-experiment-platform` is connected and its initial push is being prepared. No public web deployment, Expo project link, or TestFlight build has occurred yet. The next action after the initial push is to connect web hosting. Active viewed-time measurement remains explicitly unfinished.
