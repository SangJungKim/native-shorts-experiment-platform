...
- Added a clearly labeled sample engagement CSV for researcher demonstrations. It contains exactly 10 fictional participant × activated-Post rows using the same 17-column contract as the preliminary real-data export, includes varied interaction states, and downloads as `SAMPLE_DO_NOT_ANALYZE_participant_engagement.csv`. It never queries or writes participant data.
- Prepared a separately deployable mobile participant web demo using the same Expo/React Native interface as the native application. The production web export now has an app-like full-height document shell, phone viewport metadata, a stable title/description, clean Vercel routes, and no production link to the diagnostic playback spike. Root and workspace scripts build the static `dist` output reproducibly. This does not validate mobile-browser playback or timing; physical Safari/Chrome acceptance remains required.
- Refined the shared participant feed affordances for a familiar short-video social interface using Expo's existing cross-platform symbol support: native SF Symbols on iOS and corresponding Material Symbols on web/Android provide an outlined heart that fills when liked, a speech bubble for comments, and a two-arrow repost control that changes color when selected. The controls retain accessible labels, transparent hit areas, configured-count-plus-one feedback, undo behavior, and the existing research event semantics. Because the native and web experiences share this screen, the change applies to both.
...
Milestone 5W mobile participant web deployment is in progress before returning to Milestone 6. Hosted Supabase staging project `emffazwlvifexhklhdll` is linked; all 22 repository migrations were applied, local and remote migration histories match, and remote schema lint reports no errors. Hosted anonymous authentication is enabled, and researcher UID `a9dd4f3f-d6ca-45bd-8f79-e6f37b4b2c9a` is provisioned in `public.researcher_profiles`. The private GitHub repository `SangJungKim/native-shorts-experiment-platform` is connected and pushed. The researcher web application is deployed to Vercel with hosted Supabase client configuration. The participant app is linked to Expo project `@sangjung-kims-team/native-shorts-participant` (`5080c474-c7b7-42eb-a439-49dd87f5e37e`), and its Preview environment contains the hosted Supabase URL and a masked sensitive publishable key. The participant static web export is locally validated at a phone viewport and is ready for a separate Vercel project; no public participant URL exists yet. No TestFlight build has occurred. The next action is participant-web deployment followed by physical mobile Safari/Chrome validation. Active viewed-time measurement remains explicitly unfinished.
